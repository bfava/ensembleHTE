...
print.clan_results <- function(x, max_rows = 15, ...) {
  ...
  cat("Outcome: ", x$targeted_outcome, " | ", sep = "")
  ...
  cat(sprintf("  %-*s  %10s  %10s  %10s  %10s\n",
              nw, "", "Top", "Bottom", "Else", "All"))
  cat("  ", strrep("-", nw + 46), "\n", sep = "")
  for (i in 1:nrow(out)) {
    vname <- substr(out$variable[i], 1, nw)
    cat(sprintf("  %-*s  %10.4f  %10.4f  %10.4f  %10.4f\n",
                nw, vname,
                out$mean_top[i], out$mean_bottom[i],
                out$mean_else[i], out$mean_all[i]))
    cat(sprintf("  %-*s  %10s  %10s  %10s  %10s\n",
                nw, "",
                sprintf("(%.4f)", out$se_top[i]),
                sprintf("(%.4f)", out$se_bottom[i]),
                sprintf("(%.4f)", out$se_else[i]),
                sprintf("(%.4f)", out$se_all[i])))
  }
  
  # --- Panel 2: Differences from Top Group ---
  cat(sprintf("\nDifferences from Top Group:\n"))
  cat(sprintf("  %-*s  %12s  %12s  %12s\n",
              nw, "", "Top-Bot", "Top-Else", "Top-All"))
  cat("  ", strrep("-", nw + 42), "\n", sep = "")
  for (i in 1:nrow(out)) {
    vname <- substr(out$variable[i], 1, nw)
    ...
    cat(sprintf("  %-*s  %9.4f%-3s  %9.4f%-3s  %9.4f%-3s\n",
                nw, vname,
                out$diff_top_bottom[i], s_tb,
                out$diff_top_else[i], s_te,
                out$diff_top_all[i], s_ta))
    cat(sprintf("  %-*s  %10s    %10s    %10s\n",
                nw, "",
                sprintf("(%.4f)", out$se_diff_top_bottom[i]),
                sprintf("(%.4f)", out$se_diff_top_else[i]),
                sprintf("(%.4f)", out$se_diff_top_all[i])))
  }
