...
#' @keywords internal
.blp_pred_single <- function(Y, predicted_y, cluster_id = NULL) {
...
blp_pred <- function(ensemble_fit, outcome = NULL, subset = NULL) {
  ...
  # Unified subset handling for both HTE and pred fits
  if (is.null(subset)) {
    ...
    if (subset == "train") {
      ...
      stop("subset must be NULL, 'train', 'all', a logical vector, or integer indices")
    }
...
#'
#' @description
#' Creates a coefficient plot showing BLP prediction estimates (intercept and
#' prediction loading) with confidence intervals. Each coefficient is shown in
#' its own panel with an independent y-axis scale. A dashed reference line marks
#' the calibration target: 0 for the intercept and 1 for the prediction loading.
#'
#' @param x An object of class \code{blp_pred_results} from \code{blp_pred()}
...
#' @export
plot.blp_pred_results <- function(x, alpha = 0.05, ...) {
  ...
  plot_data[, `:=`(
    ...
    label     = ifelse(term == "intercept", "Intercept (target: 0)", "Beta (target: 1)"),
    ref_value = ifelse(term == "intercept", 0, 1)
  )]
  # Fix panel order: intercept first, beta second
  plot_data[, label := factor(label, levels = c("Intercept (target: 0)", "Beta (target: 1)"))]
  ...
  p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = "", y = estimate)) +
    ...
    ggplot2::labs(
      title    = "BLP Calibration (Prediction)",
      subtitle = paste0("Outcome: ", x$outcome,
                        if (x$outcome != x$targeted_outcome)
                          paste0(" (predictions based on: ", x$targeted_outcome, ")") else ""),
      x        = NULL,
      y        = "Estimate",
      caption  = paste0(conf_level, "% confidence intervals. ", x$M,
                        " repetitions. Dashed line: calibration target (0 for intercept, 1 for beta).")
    ) +
