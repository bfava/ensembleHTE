...
#' @param cluster_id Optional vector of cluster identifiers for cluster-robust
#'   cross-covariance. When provided, computes the meat matrix by summing
#'   score contributions within clusters (analogous to \code{sandwich::vcovCL}).
#' 
...
.calc_cov_cross_reg <- function(X1, X2, resid1, resid2, weights = NULL, 
  ...
  # --- Edge-case guards ---
  if (n <= k1) {
    stop("Cross-covariance: number of observations (n = ", n,
         ") must exceed the number of parameters in regression 1 (k = ", k1, ").")
  }
  if (n <= k2) {
    stop("Cross-covariance: number of observations (n = ", n,
         ") must exceed the number of parameters in regression 2 (k = ", k2, ").")
  }
  
  if (is.null(cluster_id)) {
    # --- HC1 (heteroskedasticity-robust) cross-covariance ---
    if (is.null(weights)) {
    ...
    # Symmetric version: geometric mean of n/(n-k1) and n/(n-k2)
    # Reduces to n/(n-k) when k1 = k2
    hc1 <- sqrt(n / (n - k1)) * sqrt(n / (n - k2))
    
    cov <- hc1 * bread1 %*% meat %*% bread2
  } else {
    # --- Cluster-robust cross-covariance ---
    clusters <- unique(cluster_id)
    G <- length(clusters)
    
    if (G <= 1) {
      stop("Cross-covariance: number of clusters (G = ", G,
           ") must be greater than 1 for cluster-robust inference.")
    }
    
    if (is.null(weights)) {
      bread1 <- solve(crossprod(X1))
      bread2 <- solve(crossprod(X2))
      # Meat: sum of outer products of cluster-level score sums
      # score1_c = sum_{i in c} X1_i * e1_i, score2_c = sum_{i in c} X2_i * e2_i
      meat <- matrix(0, ncol(X1), ncol(X2))
      for (cl in clusters) {
        idx <- which(cluster_id == cl)
        score1_c <- colSums(X1[idx, , drop = FALSE] * resid1[idx])
        score2_c <- colSums(X2[idx, , drop = FALSE] * resid2[idx])
        meat <- meat + tcrossprod(score1_c, score2_c)
      }
    } else {
      bread1 <- solve(crossprod(X1, X1 * weights))
      bread2 <- solve(crossprod(X2, X2 * weights))
      ...
      for (cl in clusters) {
        idx <- which(cluster_id == cl)
        score1_c <- colSums(X1[idx, , drop = FALSE] * (weights[idx] * resid1[idx]))
        score2_c <- colSums(X2[idx, , drop = FALSE] * (weights[idx] * resid2[idx]))
        meat <- meat + tcrossprod(score1_c, score2_c)
      }
    }
    
    # CR1 correction for clusters (consistent with sandwich::vcovCL type="HC1",
    # cadjust=TRUE): G/(G-1) * (n-1)/(n-k).
    # Symmetric version for two regressions with potentially different k:
    # G/(G-1) * sqrt((n-1)/(n-k1)) * sqrt((n-1)/(n-k2))
    # Reduces to G/(G-1) * (n-1)/(n-k) when k1 = k2
    hc1 <- (G / (G - 1)) * sqrt((n - 1) / (n - k1)) * sqrt((n - 1) / (n - k2))
...
.calc_se_diff_regs <- function(reg1, reg2, coef_name) {
  ...
  # Compute cross-covariance (passing weights and cluster_id for consistency)
  cross_cov <- .calc_cov_cross_reg(
    reg1$model_matrix, reg2$model_matrix,
    reg1$residuals, reg2$residuals,
    reg1$weights, reg1$cluster_id
  )
...
.compute_reg_diff <- function(res1, res2, n_groups) {
  ...
  # Compute cross-covariance matrix (passing weights and cluster_id for consistency)
  cross_cov <- .calc_cov_cross_reg(
    reg1$model_matrix, reg2$model_matrix,
    reg1$residuals, reg2$residuals,
    reg1$weights, reg1$cluster_id
...
#' predictions within groups defined by a restrict_by variable). Both strategies
#' use the **same** ensemble fit — they differ only in how observations are
#' ranked into groups. 
#' 
...
#' @param restrict_by The variable defining groups for restricted ranking:
#'   \itemize{
#'     \item Character string: column name in the \code{data} used when fitting the
#'       ensemble model. The variable must exist in that data at the time of fitting;
#'       columns added to the data.frame after fitting are not available.
...
#'   See \code{\link{gavs}} for details.
#' 
#' @return An object of class \code{gavs_restricted_results} containing:
#' \itemize{
...
#' 
#' @export
gavs_restricted <- function(ensemble_fit, restrict_by, n_groups = 3, outcome = NULL, 
  ...
  if (is.character(restrict_by) && length(restrict_by) == 1) {
    ...
    strata_var <- "custom_restrict_by"
  } else {
    stop("restrict_by must be a column name (string) or a vector (numeric/factor/character)")
  }
  
  # Convert restrict_by to factor if not already
  if (!is.factor(strata_vec)) {
    strata_vec <- as.factor(strata_vec)
  }
  strata_levels <- levels(strata_vec)
  
  # Validate that restrict_by is categorical (not too many unique values)
  # If restrict_by has more than 20 unique values, it's likely a continuous variable
  max_strata_levels <- 20
  if (length(strata_levels) > max_strata_levels) {
    stop(paste0("restrict_by variable '", strata_var, "' has ", length(strata_levels), 
                " unique values. This suggests it may be a continuous variable. ",
                "restrict_by should be a categorical variable (factor, integer, or character) ",
  ...
  # Also warn if restrict_by has very few observations per level
  min_obs_per_level <- min(table(strata_vec))
  if (min_obs_per_level < 10) {
    warning(paste0("Some restrict_by levels have very few observations (min = ", 
  ...
  # Validate no NAs in restrict_by for used observations
  ...
  check_strata_rest <- strata_vec[check_ref]
  ...
  all_unrest_tb <- rbindlist(lapply(results_by_rep, function(x) x$unrestricted$top_bottom), 
                              idcol = "repetition")
  unrest_tb <- all_unrest_tb[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_unrest_all <- rbindlist(lapply(results_by_rep, function(x) x$unrestricted$all), 
                               idcol = "repetition")
  unrest_all <- all_unrest_all[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_unrest_ta <- rbindlist(lapply(results_by_rep, function(x) x$unrestricted$top_all), 
                              idcol = "repetition")
  unrest_ta <- all_unrest_ta[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_rest_tb <- rbindlist(lapply(results_by_rep, function(x) x$restricted$top_bottom), 
                            idcol = "repetition")
  rest_tb <- all_rest_tb[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_rest_all <- rbindlist(lapply(results_by_rep, function(x) x$restricted$all), 
                             idcol = "repetition")
  rest_all <- all_rest_all[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_rest_ta <- rbindlist(lapply(results_by_rep, function(x) x$restricted$top_all), 
                            idcol = "repetition")
  rest_ta <- all_rest_ta[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_diff_tb <- rbindlist(lapply(results_by_rep, function(x) x$diff_results$top_bottom_diff), 
                            idcol = "repetition")
  diff_tb <- all_diff_tb[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_diff_all <- rbindlist(lapply(results_by_rep, function(x) x$diff_results$all_diff), 
                             idcol = "repetition")
  diff_all <- all_diff_all[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  all_diff_ta <- rbindlist(lapply(results_by_rep, function(x) x$diff_results$top_all_diff), 
                            idcol = "repetition")
  diff_ta <- all_diff_ta[, .(estimate = mean(estimate), se = mean(se), n_reps = .N)]
  ...
  # Construct gavs_restricted_results object
  structure(
    list(
    ...
    class = "gavs_restricted_results"
  )
}


#' Print method for gavs_restricted_results objects
#' @param x An object of class \code{gavs_restricted_results} from \code{gavs_restricted()}
#' @param ... Additional arguments (currently unused)
#' @export
print.gavs_restricted_results <- function(x, ...) {
  cat("\n")
...
#' @export
plot.gavs_restricted_results <- function(x, alpha = 0.05, ...) {
...
#'   cutoffs used to form groups. One of \code{"auto"} (default),
#'   \code{"all"}, or \code{"analysis"}.
#'   See \code{\link{gates}} for details.
...
#' 
#' @return An object of class \code{gates_restricted_results} containing:
#' \itemize{
...
gates_restricted <- function(ensemble_fit, restrict_by, n_groups = 3, outcome = NULL, 
  ...
  # Construct gates_restricted_results object
  structure(
    list(
      unrestricted = unrestricted,
      restricted = restricted,
    ...
    class = "gates_restricted_results"
  )
}


#' Print method for gates_restricted_results objects
#' @param x An object of class \code{gates_restricted_results} from \code{gates_restricted()}
#' @param ... Additional arguments (currently unused)
#' @export
print.gates_restricted_results <- function(x, ...) {
  cat("\n")
...
#'   labs theme_minimal theme element_text scale_x_continuous position_dodge
#' @export
plot.gates_restricted_results <- function(x, alpha = 0.05, ...) {
