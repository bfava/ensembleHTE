# Package index

## All functions

- [`blp()`](https://bfava.github.io/ensembleHTE/reference/blp.md) :
  Compute BLP (Best Linear Predictor) of CATE
- [`blp_pred()`](https://bfava.github.io/ensembleHTE/reference/blp_pred.md)
  : Compute BLP (Best Linear Predictor) for Prediction
- [`clan()`](https://bfava.github.io/ensembleHTE/reference/clan.md) :
  Compute CLAN (Classification Analysis)
- [`combine_ensembles()`](https://bfava.github.io/ensembleHTE/reference/combine_ensembles.md)
  : Combine Multiple Ensemble Objects
- [`ensemble_hte()`](https://bfava.github.io/ensembleHTE/reference/ensemble_hte.md)
  : Fit Ensemble Heterogeneous Treatment Effects Model
- [`ensemble_news()`](https://bfava.github.io/ensembleHTE/reference/ensemble_news.md)
  : Check for ensembleHTE Updates
- [`ensemble_pred()`](https://bfava.github.io/ensembleHTE/reference/ensemble_pred.md)
  : Fit Ensemble Prediction Model
- [`gates()`](https://bfava.github.io/ensembleHTE/reference/gates.md) :
  Compute GATES (Group Average Treatment Effects)
- [`gates_restricted()`](https://bfava.github.io/ensembleHTE/reference/gates_restricted.md)
  : Compare GATES: Unrestricted vs Restricted Ranking
- [`gavs()`](https://bfava.github.io/ensembleHTE/reference/gavs.md) :
  Compute GAVS (Group Averages)
- [`gavs_restricted()`](https://bfava.github.io/ensembleHTE/reference/gavs_restricted.md)
  : Compare GAVS: Unrestricted vs Restricted Ranking
- [`ite()`](https://bfava.github.io/ensembleHTE/reference/ite.md) :
  Extract ITE Predictions as a Matrix
- [`microcredit`](https://bfava.github.io/ensembleHTE/reference/microcredit.md)
  : Microcredit Experiment in the Philippines
- [`microcredit_covariates`](https://bfava.github.io/ensembleHTE/reference/microcredit_covariates.md)
  : Control Covariates for the Microcredit Dataset
- [`plot(`*`<blp_pred_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/plot.blp_pred_results.md)
  : Plot method for blp_pred_results objects
- [`plot(`*`<blp_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/plot.blp_results.md)
  : Plot method for blp_results objects
- [`plot(`*`<clan_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/plot.clan_results.md)
  : Plot method for clan_results objects
- [`plot(`*`<gates_restricted_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/plot.gates_restricted_results.md)
  : Plot method for gates_restricted_results objects
- [`plot(`*`<gates_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/plot.gates_results.md)
  : Plot method for gates_results objects
- [`plot(`*`<gavs_restricted_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/plot.gavs_restricted_results.md)
  : Plot method for gavs_restricted_results objects
- [`plot(`*`<gavs_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/plot.gavs_results.md)
  : Plot method for gavs_results objects
- [`print(`*`<blp_pred_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.blp_pred_results.md)
  : Print method for blp_pred_results objects
- [`print(`*`<blp_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.blp_results.md)
  : Print method for blp_results objects
- [`print(`*`<clan_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.clan_results.md)
  : Print method for clan_results objects
- [`print(`*`<ensemble_hte_fit>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.ensemble_hte_fit.md)
  : Print Method for ensemble_hte_fit Objects
- [`print(`*`<ensemble_pred_fit>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.ensemble_pred_fit.md)
  : Print Method for ensemble_pred_fit Objects
- [`print(`*`<gates_restricted_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.gates_restricted_results.md)
  : Print method for gates_restricted_results objects
- [`print(`*`<gates_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.gates_results.md)
  : Print method for gates_results objects
- [`print(`*`<gavs_restricted_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.gavs_restricted_results.md)
  : Print method for gavs_restricted_results objects
- [`print(`*`<gavs_results>`*`)`](https://bfava.github.io/ensembleHTE/reference/print.gavs_results.md)
  : Print method for gavs_results objects
- [`summary(`*`<ensemble_hte_fit>`*`)`](https://bfava.github.io/ensembleHTE/reference/summary.ensemble_hte_fit.md)
  : Summary Method for ensemble_hte_fit Objects
- [`summary(`*`<ensemble_pred_fit>`*`)`](https://bfava.github.io/ensembleHTE/reference/summary.ensemble_pred_fit.md)
  : Summary Method for ensemble_pred_fit Objects
