...
### Using CLAN with Subsets

When using `subset`, the `group_on` argument controls which observations define the group cutoffs. This applies to `gates()`, `gavs()`, `clan()`, `gates_restricted()`, and `gavs_restricted()`:

- `group_on = "auto"` (default): Groups are formed using the same population that was used to **train** the ML model. For `ensemble_hte` fits this means all observations; for `ensemble_pred` fits trained on a subset (via `train_idx`) this means only the training subset. The analysis (means for CLAN, regressions for GATES/GAVS) is then run on the analysis subset. This default ensures that group definitions reflect the population the ML model was designed for.
- `group_on = "all"`: Groups are always formed using **all** observations, regardless of how the ML was trained.
- `group_on = "analysis"`: Groups are formed within the analysis subset only.

```{r clan-subset, eval=FALSE}
# Suppose some observations have missing data on a variable
observed <- !is.na(data$Z_partial)

# Default (auto): group cutoffs come from the ML training population
clan_all_vars <- clan(fit, variables = c("X1", "X2"), subset = observed)
clan_partial  <- clan(fit, variables = c("Z_partial"),  subset = observed)
# Both use the same group assignments — an observation that is "top" in one
# is also "top" in the other.

# Alternative: form groups only within the observed subset
clan_reranked <- clan(fit, variables = c("Z_partial"), subset = observed,
                      group_on = "analysis")
```
...
The `gates_restricted()` function formally tests whether these strategies yield different outcomes:
...
# Compare strategies
comparison <- gates_restricted(fit, restrict_by = "region", n_groups = 3)
print(comparison)
```
...
Like `gates_restricted()`, you can compare unrestricted vs. restricted prediction strategies:

```{r gavs-compare}
# Compare prediction strategies by region
gavs_comparison <- gavs_restricted(fit_pred, restrict_by = "region", n_groups = 3)
...
The `train_idx` parameter is also available in `ensemble_hte()`, which is useful for **multi-arm trials**. For example, if you have three treatment arms (A, B, C), you can estimate heterogeneous treatment effects between arms A and B, then predict ITEs for everyone (including arm C):

```{r hte-train-idx}
# Simulate a 3-arm trial
set.seed(42)
n <- 1500
X1 <- rnorm(n)
X2 <- rnorm(n)
arm <- sample(c("A", "B", "C"), n, replace = TRUE)

# D = 1 for arm B, D = 0 for arm A (arm C is "other")
D_AB <- ifelse(arm == "B", 1, ifelse(arm == "A", 0, NA))

# Outcome (treatment effect only between A and B)
tau <- 1 + 0.8 * X1
Y <- 0.5 * X1 + 0.3 * X2 + ifelse(arm == "B", tau, 0) + rnorm(n, sd = 0.5)

data_multi <- data.frame(Y = Y, D = ifelse(is.na(D_AB), 0, D_AB),
                          X1 = X1, X2 = X2, arm = arm)

# Fit HTE on arms A and B only, but predict ITEs for everyone
fit_multi <- ensemble_hte(
  formula = Y ~ X1 + X2,
  treatment = D,
  data = data_multi,
  train_idx = (arm %in% c("A", "B")),
  algorithms = c("lm", "grf"),
  M = 100, K = 3
)

# GATES on training observations (arms A and B)
gates(fit_multi)

# GATES on all observations (including arm C)
gates(fit_multi, subset = "all")
```
...
5. **Algorithm choice**: `lm` and `grf` are fast; `xgboost` with tuning is slower; `nnet` and `svm` can be slow on large data.
...
4. **Use `gates_restricted()` to evaluate targeting strategies**
...
# Simulate panel data: 200 individuals observed over 3 time periods
set.seed(42)
n_individuals <- 200
n_periods <- 3
...
individual_id <- rep(1:n_individuals, each = n_periods)
time_period <- rep(1:n_periods, times = n_individuals)

# Individual-level covariates (fixed across time)
X1_ind <- rnorm(n_individuals)
X2_ind <- rbinom(n_individuals, 1, 0.5)
X1 <- rep(X1_ind, each = n_periods)
X2 <- rep(X2_ind, each = n_periods)

# Time-varying covariate
X3 <- rnorm(n)

# Treatment (randomized at individual level)
D_ind <- rbinom(n_individuals, 1, 0.5)
D <- rep(D_ind, each = n_periods)

# Heterogeneous treatment effect: tau(X) = 1 + 0.5*X1
tau <- 1 + 0.5 * X1

# Outcome with individual fixed effects and time effects
alpha_i <- rep(rnorm(n_individuals, 0, 0.5), each = n_periods)  # individual FE
Y <- 2 + alpha_i + 0.3 * X1 + 0.2 * X3 + tau * D + rnorm(n)

# Create data frame
panel_data <- data.frame(
  Y = Y, D = D, X1 = X1, X2 = X2, X3 = X3,
  individual_id = individual_id, time = time_period
)
...
fit <- ensemble_hte(
  ...
  data = panel_data,
  treatment = D,
  individual_id = individual_id,  # Specify the individual identifier
